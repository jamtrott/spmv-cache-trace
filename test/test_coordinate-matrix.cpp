...
{
    coo_matrix::Matrix::index_array_type row_index{{0u, 0u, 1u, 2u, 3u, 3u}};
    coo_matrix::Matrix::index_array_type column_index{{0u, 1u, 1u, 2u, 3u, 4u}};
    coo_matrix::Matrix::value_array_type value{{1.0, 2.0, 1.0, 3.0, 2.0, 1.0}};
    return coo_matrix::Matrix(
        4u, 5u, 6u, coo_matrix::Order::general,
...
{
    auto A = testMatrix();
    auto x = coo_matrix::Matrix::value_array_type{{5.0, 2.0, 3.0, 3.0, 1.0}};
    auto y = A * x;
    auto z = coo_matrix::Matrix::value_array_type{{9.0, 2.0, 9.0, 7.0}};
...
TEST(coordinate_matrix, matrix_vector_multiplication_column_major)
{
    ...
    auto x = coo_matrix::Matrix::value_array_type{{5.0, 2.0, 3.0, 3.0, 1.0}};
    auto y = A * x;
    auto z = coo_matrix::Matrix::value_array_type{{18.0, 22.0, 9.0, 7.0}};
    ASSERT_DOUBLE_EQ(l2norm(y - z), 0.0)
        << "A = " << A << ",\n" << "x = " << x << ",\n"
        << "y = " << y << ",\n" << "z = " << z;
}
...
TEST(coordinate_matrix, poisson2D)
{
    ...
    auto x = coo_matrix::Matrix::value_array_type{
        std::cbegin(poisson2D_b), std::cend(poisson2D_b)};
    auto y = A * x;
    auto z = coo_matrix::Matrix::value_array_type{
