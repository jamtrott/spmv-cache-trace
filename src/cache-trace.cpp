...
CacheTrace::CacheTrace(
    ...
    Kernel const & kernel,
    std::map<std::string, std::vector<cache_miss_type>> const & cache_misses)
...
std::map<std::string, std::vector<cache_miss_type>> const &
    ...
    Cache const & cache)
{
    ...
    for (int i = 0; i < num_threads; i++) {
        auto it = caches.find(thread_affinities[i]);
        if (it == caches.end()) {
            std::stringstream s;
...
std::vector<cache_miss_type> trace_cache_misses_per_cache(
    TraceConfig const & trace_config,
    Kernel const & kernel,
    Cache const & cache)
{
    ...
    std::vector<cache_miss_type> active_threads_cache_misses =
        replacement::cost(
            replacement_algorithm,
            memory_reference_strings);

    std::vector<cache_miss_type> cache_misses(num_threads);
...
CacheTrace trace_cache_misses(
    ...
    Kernel const & kernel)
{
    std::map<std::string, std::vector<cache_miss_type>> cache_misses;
    ...
    for (auto it = std::cbegin(caches); it != std::cend(caches); ++it) {
        ...
        std::vector<cache_miss_type> num_cache_misses_per_thread =
            trace_cache_misses_per_cache(
                trace_config, kernel, cache);
        cache_misses.emplace(cache.name, num_cache_misses_per_thread);
    }
